...
from ppu cimport Ppu, NUM_SPRITES_IN_OAM, BYTES_PER_SPRITE, INVISIBLE_SPRITE, SPRITE_Y_OFFSET, SPRITE_X_OFFSET, SPRITE_ATTRIBUTE_OFFSET, SPRITE_TILE_INDEX_OFFSET, PALETTE_BASE_ADDR, SPRITE_PALETTE_BASE_ADDR, NUM_BACKGROUND_PALETTES, NUM_SPRITE_PALETTES, NUM_BYTES_PER_PALETTE
...
def insert_background_palette(Ppu ppu, nes_image_palette):
...
def insert_sprite_palette(Ppu ppu, nes_image_palette):
...
def hide_all_sprites(Ppu ppu):
    for i in range(0, NUM_SPRITES_IN_OAM):
        ppu.oam[BYTES_PER_SPRITE*i + SPRITE_Y_OFFSET] = INVISIBLE_SPRITE

def insert_sprite(Ppu ppu, sprite_index, y, x, tile_index, palette_index):
