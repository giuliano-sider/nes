...
class TestLoad(unittest.TestCase):
    ...
    def test_if_register_a_receives_immediate(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDA_IMMEDIATE, op2_lo_byte=0x01)
        expected_pc = initial_pc + 2

        self.assertEqual((<Cpu> self.cpu).A(), 0x01)
    ...
    def test_if_negative_flag_is_cleared_if_operand_is_between_x00_and_x7f_for_lda_immediate(self):
        (<Cpu> self.cpu).set_A(0x03)

        execute_instruction((<Cpu> self.cpu), opcode=LDA_IMMEDIATE, op2_lo_byte=0x01)
        expected_value = 0
        self.assertEqual(expected_value, (<Cpu> self.cpu).negative())

    def test_if_negative_flag_is_not_cleared_if_operand_is_greater_than_x7f_for_lda_immediate(self):
        (<Cpu> self.cpu).set_A(0x03)

        execute_instruction((<Cpu> self.cpu), opcode=LDA_IMMEDIATE, op2_lo_byte=0x88)
        expected_value = 1
        self.assertEqual(expected_value, (<Cpu> self.cpu).negative())
    ...
    def test_if_zero_flag_is_set_when_operand_is_zero_for_lda_immediate(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDA_IMMEDIATE, op2_lo_byte=0x00)
        expected_value = 1
        self.assertEqual(expected_value, (<Cpu> self.cpu).zero())
    ...
    def test_zero_page_lda(self):
        ...
        (<Cpu> self.cpu).set_A(0x03)
        (<Cpu> self.cpu).set_memory(storage_address, stored_content)

        execute_instruction((<Cpu> self.cpu), opcode=LDA_ZEROPAGE, op2_lo_byte=storage_address)

        expected_pc = initial_pc + 2
        expected_value = stored_content
        self.assertEqual(expected_value, (<Cpu> self.cpu).A())
        self.assertEqual(expected_pc, (<Cpu> self.cpu).PC())
    ...
    def test_zero_page_when_content_is_zero(self):
        ...
        self.assertEqual(expected_zero_flag_value, (<Cpu> self.cpu).zero())
    ...
    def test_zero_page_when_content_is_negative(self):
        ...
        self.assertEqual(expected_negative_flag_value, (<Cpu> self.cpu).negative())
    ...
    def test_lda_absolute(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDA_ABSOLUTE, op2_lo_byte=low_address_part, op2_hi_byte=high_address_part)
        expected_pc = initial_pc + 3
        expected_stored_value = stored_content
        self.assertEqual(expected_stored_value, (<Cpu> self.cpu).A())
        self.assertEqual(expected_pc, (<Cpu> self.cpu).PC())
    ...
    def test_zero_flag_for_lda_absolute_instruction(self):
        ...
        (<Cpu> self.cpu).set_A(any_content)
        (<Cpu> self.cpu).set_memory(storage_address, stored_content)

        execute_instruction((<Cpu> self.cpu), opcode=LDA_ABSOLUTE, op2_lo_byte=low_address_part, op2_hi_byte=high_address_part)
        expected_zero_flag = 1
        self.assertEqual(expected_zero_flag, (<Cpu> self.cpu).zero())
    ...
    def test_negative_flag_when_lda_absolute_instruction(self):
        ...
        self.assertEqual(expected_negative_flag, (<Cpu> self.cpu).negative())
    ...
    def test_lda_indirect_y_without_overflow(self):
        ...
        (<Cpu> self.cpu).set_memory(stored_address_in_0x86 + y_value, stored_content)
        (<Cpu> self.cpu).set_memory(zero_page_address, stored_address_in_0x86_low)
        (<Cpu> self.cpu).set_memory(zero_page_address + 1, stored_address_in_0x86_high)

        expected_value = stored_content
        expected_pc = initial_pc + 2

        execute_instruction((<Cpu> self.cpu), opcode=LDA_INDIRECT_Y, op2_lo_byte=zero_page_address)
    ...
    def test_lda_indirect_y_without_overflow_hi_part(self):
        ...
        (<Cpu> self.cpu).set_memory((stored_address_in_0x86 + y_value) % 0x10000, stored_content)
        (<Cpu> self.cpu).set_memory(zero_page_address, stored_address_in_0x86_low)
        (<Cpu> self.cpu).set_memory(zero_page_address + 1, stored_address_in_0x86_high)
    ...
    def test_lda_indirect_x_add_value_to_zero_page_address(self):
        ...
        (<Cpu> self.cpu).set_memory(zero_page_address, indirect_zero_page_address)
        (<Cpu> self.cpu).set_memory(resolved_address, stored_content)
        execute_instruction((<Cpu> self.cpu), opcode=LDA_INDIRECT_X, op2_lo_byte=zero_page_address)
    ...
    def test_lda_absolute_y_without_overflow(self):
        ...
        execute_instruction((<Cpu> self.cpu),
    ...
    def test_zero_page_x_lda_without_overflow(self):
        ...
        (<Cpu> self.cpu).set_X(x_value)
        (<Cpu> self.cpu).set_memory(storage_address + x_value, stored_content)

        execute_instruction((<Cpu> self.cpu), opcode=LDA_ZEROPAGE_X, op2_lo_byte=storage_address)
        expected_value = stored_content
        expected_pc = initial_pc + 2
    ...
    def test_zero_page_x_lda_with_overflow(self):
        ...
        (<Cpu> self.cpu).set_X(x_value)
        (<Cpu> self.cpu).set_memory(0x00, stored_content)
    ...
    def test_if_register_x_receives_immediate(self):
        stored_content = 0x01
        execute_instruction((<Cpu> self.cpu), opcode=LDX_IMMEDIATE, op2_lo_byte=stored_content)
        expected_value = stored_content
        expected_negative_flag = 0
        expected_zero_flag = 0
        self.assertEqual(expected_value, (<Cpu> self.cpu).X())
        self.assertEqual(expected_zero_flag, (<Cpu> self.cpu).zero())
        self.assertEqual(expected_negative_flag, (<Cpu> self.cpu).negative())
    ...
    def test_zero_page_ldx(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDX_ZEROPAGE, op2_lo_byte=storage_address)
        expected_value = stored_content
        expected_negative_flag = 0
        expected_zero_flag = 0
        expected_pc = initial_pc + 2
    ...
    def test_ldx_absolute(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDX_ABSOLUTE, op2_lo_byte=low_address_part, op2_hi_byte=high_address_part)
        expected_value = stored_content
        expected_negative_flag = 0
        expected_zero_flag = 0
        expected_pc = initial_pc + 3
    ...
    def test_zero_page_ldy(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDY_ZEROPAGE, op2_lo_byte=storage_address)
        expected_value = stored_content
        expected_negative_flag = 0
        expected_zero_flag = 0
        expected_pc = initial_pc + 2

        self.assertEqual(expected_value, (<Cpu> self.cpu).Y())
        self.assertEqual(expected_zero_flag, (<Cpu> self.cpu).zero())
        self.assertEqual(expected_negative_flag, (<Cpu> self.cpu).negative())
        self.assertEqual(expected_pc, (<Cpu> self.cpu).PC())
    ...
    def test_ldy_absolute(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDY_ABSOLUTE, op2_lo_byte=low_address_part, op2_hi_byte=high_address_part)
        expected_value = stored_content
        expected_negative_flag = 0
        expected_zero_flag = 0
        expected_pc = initial_pc + 3
    ...
    def test_ldx_zero_page_y_without_overflow(self):
        ...
        (<Cpu> self.cpu).set_Y(y_value)
        (<Cpu> self.cpu).set_memory(storage_address + y_value, stored_content)

        execute_instruction((<Cpu> self.cpu), opcode=LDX_ZEROPAGE_Y, op2_lo_byte=storage_address)
        expected_value = stored_content
        expected_negative_flag = 0
        expected_zero_flag = 0
        expected_pc = initial_pc + 2
    ...
    def test_ldx_zero_page_y_with_overflow(self):
        ...
        (<Cpu> self.cpu).set_Y(y_value)
        (<Cpu> self.cpu).set_memory((storage_address + y_value) % 256, stored_content)
    ...
    def test_if_register_y_receives_immediate(self):
        stored_content = 0x01
        execute_instruction((<Cpu> self.cpu), opcode=LDY_IMMEDIATE, op2_lo_byte=stored_content)
        expected_value = stored_content
        expected_negative_flag = 0
        expected_zero_flag = 0
        self.assertEqual(expected_value, (<Cpu> self.cpu).Y())
    ...
    def test_ldy_zero_page_x_without_overflow(self):
        ...
        execute_instruction((<Cpu> self.cpu), opcode=LDY_ZEROPAGE_X, op2_lo_byte=storage_address)
        expected_value = stored_content
        expected_pc = initial_pc + 2
