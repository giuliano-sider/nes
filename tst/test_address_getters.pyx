...
from Instructions.address_getters cimport get_indirect_x, get_indirect_y
...
class TestAddressGetters(unittest.TestCase):
    ...
    def test_get_indirect_x_no_wraparound(self):
        ...
        cpu.set_memory(0xFE, 0x10)
        cpu.set_memory(0xFF, 0x02)
        cpu.set_memory(0x0210, 0xAE)
        # wrong address is here
        cpu.set_memory(0x00, 0x12)
        cpu.set_memory(0x01, 0x01)
        cpu.set_memory(0x0112, 0xAF)
        # LDA (0x00, X)
    ...
    def test_get_indirect_x_wraparound_x(self):
        ...
        cpu.set_X(0xFF)
        # correct address is here
        cpu.set_memory(0x00, 0x00)
        cpu.set_memory(0x01, 0x02)
        cpu.set_memory(0x0200, 0xAE)
        # wrong address is here
        cpu.set_memory(0x100, 0x12)
        cpu.set_memory(0x101, 0x03)
        cpu.set_memory(0x0312, 0xAF)
        # LDA (0x01, X)
    ...
    def test_get_indirect_x_wraparound_indirect_address(self):
        ...
        cpu.set_X(0xFE)
        # correct address is here
        cpu.set_memory(0xFF, 0x00)
        cpu.set_memory(0x00, 0x02)
        cpu.set_memory(0x0200, 0xAE)
        # wrong address is here
        cpu.set_memory(0x100, 0x03)
        cpu.set_memory(0x0300, 0xAF)
        # LDA (0x01, X)
        insert_instruction(cpu, cpu.PC(), opcode=LDA_INDIRECT_X, op2_lo_byte=0x01)
    ...
    def test_get_indirect_y_no_wraparound(self):
        ...
        cpu.set_memory(0x00, 0x00)
        cpu.set_memory(0x01, 0x02)
        cpu.set_memory(0x0200 + 0xFE, 0xAE)
        # wrong value is here
        cpu.set_memory(0x0200, 0xAF)
        # LDA (0x00), Y
    ...
    def test_get_indirect_y_wraparound_indirect_address(self):
        ...
        # wrong value is here
        cpu.set_memory(0x100, 0x03)
        cpu.set_memory(0x0300 + 0xFE, 0xAF)
        # LDA (0xFF), Y
